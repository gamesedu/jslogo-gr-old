...
function LogoInterpreter(turtle, stream, savehook)
{
  ...
  def(["forward", "fd","ΜΠΡΟΣΤΑ", "ΜΠ"], function(a) { return turtle.move(aexpr(a)); });
  def(["back", "bk","ΠΙΣΩ", "ΠΙ"], function(a) { return turtle.move(-aexpr(a)); });
  ...
  def(["clearscreen", "cs","ΚΑΘΑΡΙΣΕ_ΟΘΟΝΗ","ΝΕΟ","ΚΟ","CLS"], function() { return turtle.clearscreen(); });
  ...
  //
  def(["pendown", "pd","ΣΤΥΛΟ_ΚΑΤΩ","ΣΤΚ"], function() { return turtle.pendown(); });
  def(["penup", "pu","ΣΤΥΛΟ_ΑΝΩ","ΣΤΑ"], function() { return turtle.penup(); });
  ...
  def(["setpensize", "setwidth", "setpw","ΜΕΓΕΘΟΣ_ΣΤΥΛΟ","ΠΑΧΟΣ"], function(a) {
    if (Type(a) === 'list') {
  ...
  def(["repeat","ΕΠΑΝΕΛΑΒΕ","ΕΠΑΝΑΛΑΒΕ"], function(count, statements) {
    count = aexpr(count);
    statements = reparse(lexpr(statements));
