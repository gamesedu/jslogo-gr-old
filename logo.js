...
// Greek Modifications by trister 
// Ω04Ε 20160317 -aCCENTED CHARS
// v04 -20160218d transfer ownership2
// v04 -20160218c
// 
//
//
...
{
 /// 'use strict'; //20160227 jon removed "use strict" so program can work to older browsers (maybe a mistake?)
...
//-----------------------
  ...
  function parse(string) {
    ...
    string= replace_greek(string);//added by J 20160317 ########
  ...
  //
  def(["to","ΓΙΑ","ΔΙΑΔΙΚΑΣΙΑ","ΣΥΝΑΡΤΗΣΗ","function"], function(list) {
    ...
    while (list.length) {
      var atom = list.shift();
      if (isKeyword(atom, 'END') || isKeyword(atom, 'ΤΕΛΟΣ')) {
        sawEnd = true;
        break;
    ...
    if (!sawEnd) {
      throw new Error(format(__("Expected END - ΑΝΑΜΕΝΟΤΑΝ Η ΛΈΞΗ 'ΤΕΛΟΣ' ")));
    }
  ...
  def(["pick","ΕΠΕΛΕΞΕ"], function(list) {
    list = lexpr(list);
  ...
  def(["print", "pr","ΤΥΠΩΣΕ"], function(thing) {
  ...
  def(["power","ΔΥΝΑΜΗ","ΔΥ"], function(a, b) { return Math.pow(aexpr(a), aexpr(b)); });
  def(["sqrt","ΤΡΙΖΑ","ΤΕΤΡΑΓΩΝΙΚΗ_ΡΙΖΑ"], function(a) { return Math.sqrt(aexpr(a)); });
  ...
  def(["random","ΤΥΧΑΙΟ"], function(max) {
    max = aexpr(max);
  ...
  def(["true","ΑΛΗΘΕΣ"], function() { return 1; });
  def(["false","ΨΕΥΔΕΣ"], function() { return 0; });

  def(["and","ΚΑΙ"], function(a, b) {
    var args = Array.from(arguments);
    return booleanReduce(args, function(value) {return value;}, 1);
  }, {noeval: true});

  def(["or","Η","Ή"], function(a, b) {
    var args = Array.from(arguments);
  ...
  def(["forward", "fd","fW","ΜΠΡΟΣΤΑ", "ΜΠ"], function(a) { return turtle.move(aexpr(a)); });
  def(["back", "bk","bW","ΠΙΣΩ", "ΠΙ"], function(a) { return turtle.move(-aexpr(a)); });
  def(["left", "lt","ΑΡΙΣΤΕΡΑ", "ΑΡ"], function(a) { return turtle.turn(-aexpr(a)); });
  def(["right", "rt","ΔΕΞΙΑ", "ΔΕ"], function(a) { return turtle.turn(aexpr(a)); });
  ...
  def(["setheading", "seth","ΘΕΣΕ_ΠΟΡΕΙΑ","ΠΟΡΕΙΑ","ΚΑΤΕΥΘΥΝΣΗ"], function(a) { return turtle.setheading(aexpr(a)); });

  def(["home","ΑΡΧΗ"], function() { return turtle.home(); });

  def(["arc","ΤΟΞΟ","ΚΑΜΠΥΛΗ"], function(angle, radius) { return turtle.arc(aexpr(angle), aexpr(radius)); });
  ...
  def(["showturtle", "st","ΔΕΙΞΕ_ΧΕΛΩΝΑ","ΔΧ","ΕΜΦΑΝΙΣΕ_ΧΕΛΩΝΑ","ΕΧ"], function() { return turtle.showturtle(); });
  def(["hideturtle", "ht","ΚΡΥΨΕ_ΧΕΛΩΝΑ","ΚΧ"], function() { return turtle.hideturtle(); });
  def(["clean","ΚΑΘΑΡΙΣΕ"], function() { return turtle.clear(); });
  def(["clearscreen", "cs","ΚΑΘΑΡΙΣΕ_ΟΘΟΝΗ","ΝΕΟ","ΚΟ","CLS","KO"], function() { return turtle.clearscreen(); });

  def("wrap", function() { return turtle.setturtlemode('wrap'); }); //ΧΕΛΩΝΑ ΕΠΑΝΕΡΧΕΤΑΙ ΟΤΑΝ ΒΓΕΙ ΕΚΤΟΣ ΟΡΙΩΝ ΟΘΟΝΗΣ
  def("window", function() { return turtle.setturtlemode('window'); }); //ΧΕΛΩΝΑ ΒΓΑΙΝΕΙ ΕΚΤΟΣ ΟΘΟΝΗΣ
  def(["fence","ΦΡΑΧΤΗΣ"], function() { return turtle.setturtlemode('fence'); });// ΧΕΛΩΝΑ ΔΕΝ ΒΓΑΙΝΕΙ ΕΚΤΟΣ ΟΘΟΝΗΣ

  def(["fill","ΓΕΜΙΣΕ"], function() { return turtle.fill(); });

  def(["filled","ΓΕΜΑΤΟ"], function(fillcolor, statements) {
  ...
  //
  def(["pendown", "pd","ΣΤΥΛΟ_ΚΑΤΩ","ΣΤΥΛΟΚΑΤΩ","ΣΤΚ"], function() { return turtle.pendown(); });
  def(["penup", "pu","ΣΤΥΛΟ_ΑΝΩ","ΣΤΥΛΟΑΝΩ","ΣΤΑ"], function() { return turtle.penup(); });
  ...
  def(["setpencolor", "setpc", "setcolor","ΧΡΩΜΑ","ΟΡΙΣΕ_ΧΡΩΜΑ"], function(color) {
    function adjust(n) {
      // Clamp into 0...99
  ...
  def(["setpensize", "setwidth", "setpw","ΜΕΓΕΘΟΣ_ΣΤΥΛΟ","ΠΑΧΟΣ_ΣΤΥΛΟ","ΠΑΧΟΣ"], function(a) {
    if (Type(a) === 'list') {
  ...
  def(["make","ΚΑΝΕ","ΟΡΙΣΕ","ΘΕΣΕ"], function(varname, value) {
  ...
  def(["erase","ΔΙΕΓΡΑΨΕ"], function(list) {
    list = lexpr(list);
  ...
  def(["repeat","ΕΠΑΝΕΛΑΒΕ","ΕΠΑΝΑΛΑΒΕ","ΕΠΑΝΈΛΑΒΕ","ΕΠΑΝΆΛΑΒΕ"], function(count, statements) {
    count = aexpr(count);
    statements = reparse(lexpr(statements));
  ...
  def(["forever","ΓΙΑΠΑΝΤΑ"], function(statements) {
    statements = reparse(lexpr(statements));
    var old_repcount = self.repcount;
    var i = 1;
    return promiseFinally(
  ...
  def(["repcount","ΑΡΙΘΜΟΣ_ΕΠΑΝΑΛΗΨΗΣ","ΕΠΑΝΑΛΗΨΗ"], function() {
    return self.repcount;
  });

  def(["if","ΕΑΝ"], function(test, statements) {
    test = aexpr(test);
    statements = reparse(lexpr(statements));
  ...
  def(["ifelse","ΑΛΛΙΩΣ","ΕΑΝ_ΑΛΛΙΩΣ","ΑνΔιαφορετικά","εαν_διαφορετικα"], function(test, statements1, statements2) {
    test = aexpr(test);
  ...
  def(["wait","ΠΕΡΙΜΕΝΕ"], function(time) {
  ...
  def(["for","ΦΟΡ"], function(control, statements) {
  ...
  def(["while","ΟΣΟ","ΟΣΟ_ΙΣΧΥΕΙ","ΕΦΟΣΟΝ","ΕΝΩ"], function(tf, block) {
    block = checkevalblock(block);
    return promiseLoop(function(loop, resolve, reject) {
      Promise.resolve(tf())
        .then(function(cond) {
  ...
  def(["foreach","ΓΙΑ_ΚΑΘΕ","ΓΙΑΚΑΘΕ"], function(procname, list) {
    procname = sexpr(procname);
